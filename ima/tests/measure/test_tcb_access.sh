...
# Description: test_tcb_access
# Author: Zhang Tianxing <zhangtianxing3@huawei.com>
# Create: 2020/09/09

TST_TESTFUNC=test_tcb_access
TST_SETUP=setup

export TCID=test_tcb_access
...
test_tcb_access_copyfile_without_xattr() {
...
test_tcb_access_copyfile_with_xattr() {
...
test_tcb_access_context_changed() {
    cp -a $TESTFILE $TESTFILE_BAK
...
test_tcb_access_selinux_changed() {
    cp $TESTFILE $TESTFILE_BAK
    rm $TESTFILE
    mv $TESTFILE_BAK $TESTFILE
